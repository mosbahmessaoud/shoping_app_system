...
async def upload_product_images(files: List[UploadFile] = File(default=[])):
    """
...
async def delete_product_image(public_id: str):
    ...
    try:
        ...
        if result.get("result") == "ok":
            ...
            raise HTTPException(
                status_code=404, detail=f"Image '{public_id}' not found"
            )
        else:
            raise HTTPException(
                status_code=500,
    ...
    try:
        ...
        return {
            ...
            "cloudinary_configured": bool(
                config.cloud_name and config.api_key and config.api_secret
            ),
